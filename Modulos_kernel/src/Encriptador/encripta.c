...
{//Esta funcion se ejecuta al cargar el modulo
	int t = register_chrdev(89,FILE_NAME,&fops);
	if(t<0) printk(KERN_ALERT "Fallo la carga del modulo encripta.\n");
	else printk(KERN_ALERT "La carga del modulo encripta fue un exito.\n");
...
{//Esta funcion se ejecuta al descargar el modulo
	...
	printk(KERN_ALERT "La descarga del modulo encripta fue un exito.\n");
}
...
static ssize_t dev_write(struct file *filp, char *buff, size_t len, loff_t *off)
{
	...
	while(len > 0)
	{
		msg[count] = buff[count] + (char) 10; //se desplazan 10 lugares.
		ind--;
		count++;
		len--;
	}
